...
import type { CompanionState } from './nimbusCompanionState';

export type { CompanionState } from './nimbusCompanionState';
...
function StateIcon({ state, reduceMotion }: { state: CompanionState; reduceMotion: boolean }) {
  const cn = 'h-3.5 w-3.5';

  if (state === 'thinking') {
    return <LoaderCircle className={`${cn} ${reduceMotion ? '' : 'animate-spin'}`} />;
  }
...
export default function NimbusCompanion({ state }: { state: CompanionState }) {
  ...
  useEffect(() => {
    try {
      ...
      const saved = JSON.parse(window.localStorage.getItem(POSITION_KEY) || 'null') as {
        x?: unknown;
        y?: unknown;
      } | null;
    ...
    (forcedState?: CompanionState) => {
      ...
      setPose((current) => {
        if (userPlacedRef.current) {
          return { ...current, jump: current.jump + 1 };
        }
  ...
  const finishDrag = (_event: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
    ...
    setPose((current) => {
      ...
      return {
        ...
        x: droppedX,
        y: droppedY,
        facing: info.offset.x < 0 ? -1 : info.offset.x > 0 ? 1 : current.facing,
        jump: current.jump + 1,
      };
    });
  };
  ...
  const statusColor =
    state === 'error'
      ...
      : state === 'thinking' || state === 'building'
        ? 'bg-amber-300'
        : 'bg-emerald-400';
  const travelTransition = useMemo(
    () => ({ type: 'spring' as const, stiffness: state === 'building' ? 115 : 72, damping: 16, mass: 0.8 }),
    [state],
  );
      ...
      <AnimatePresence mode="wait">
        {!visible ? (
          <motion.button
            ...
            className="pointer-events-auto absolute right-1 top-2 flex h-9 w-9 appearance-none items-center justify-center rounded-full border border-white/10 bg-[#111827]/90 p-0 text-cyan-200 shadow-lg backdrop-blur"
          ...
          >
            <motion.div
              ...
              transition={reduceMotion ? { duration: 0 } : travelTransition}
            >
              <motion.span
              ...
              <motion.button
                ...
                className="absolute inset-x-0 top-0 h-[104px] w-[82px] cursor-inherit origin-bottom appearance-none border-0 bg-transparent p-0 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/60"
                initial={false}
                animate={
                  reduceMotion
                    ? { opacity: 1 }
                    : state === 'error'
                      ? {
                          ...
                          y: [0, -hop, 0],
                          rotate: [0, -3, 3, -2, 2, 0],
                          scaleX: [pose.facing, pose.facing * 0.96, pose.facing],
                          scaleY: [1, 1.02, 0.96, 1],
                        }
                      : {
                          y: [0, -hop * 0.82, -hop, -hop * 0.56, 0, 2, 0],
                          rotate: [0, pose.facing * 2, pose.facing * 4, 0, pose.facing * -2, 0],
                          scaleX: [pose.facing, pose.facing * 0.96, pose.facing, pose.facing * 1.08, pose.facing],
                          scaleY: [0.94, 1.08, 1.03, 0.96, 0.88, 1.05, 1],
                        }
                }
                transition={
                  reduceMotion
                    ? { duration: 0 }
                    : {
                        duration: state === 'done' ? 0.75 : state === 'building' ? 0.85 : 0.95,
              ...
              >
                ...
                >
                  ...
                  <span className="relative flex h-[46%] w-[68%] items-center justify-center rounded-[42%] border-2 border-cyan-100/60 bg-[#13286f] shadow-[inset_0_0_12px_rgba(34,211,238,.5)]">
                    <Bot
                      className="h-7 w-7 text-cyan-100 drop-shadow-[0_0_5px_rgba(103,232,249,.8)]"
                      strokeWidth={1.8}
                    />
                  </span>
                </span>
                ...
                {companionAssetStatus !== 'failed' ? (
                  <img
                    ...
                    className={`h-full w-full select-none object-contain [image-rendering:pixelated] drop-shadow-[0_8px_10px_rgba(34,211,238,0.22)] transition-opacity duration-200 motion-reduce:transition-none ${companionAssetStatus === 'ready' ? 'opacity-100' : 'opacity-0'}`}
                  />
                ) : null}

                {companionAssetStatus === 'ready'
                  ? [43.2, 66.4].map((left, index) => (
                      <span
                        key={left}
                        className="absolute top-[37.2%] h-[12%] w-[8.5%] overflow-hidden rounded-full"
                        style={{ left: `${left - 4.25}%` }}
                      >
                        <motion.span
                        ...
                        <motion.span
                          ...
                          transition={reduceMotion ? { duration: 0 } : { type: 'spring', stiffness: 430, damping: 24 }}
                        />
                      </span>
                    ))
                  : null}
                <span
                  className={`absolute bottom-[17%] right-[12%] h-2 w-2 rounded-full border border-[#07111B] ${statusColor}`}
                />
              </motion.button>
              ...
              <AnimatePresence>
                {reaction > 0 ? (
                  <motion.span
                    ...
                    initial={reduceMotion ? false : { opacity: 0, y: 25, scale: 0.5, rotate: -12 }}
                    animate={
                      reduceMotion
                        ? { opacity: 0 }
                        : { opacity: [0, 1, 1, 0], y: -30, scale: [0.5, 1.25, 1, 0.8], rotate: 8 }
                    }
              ...
              <AnimatePresence initial={false}>
                {expanded ? (
                  <motion.div
                    ...
                    transition={reduceMotion ? { duration: 0 } : undefined}
                  >
                    <span className={state === 'error' ? 'text-rose-300' : 'text-cyan-200'}>
                      <StateIcon state={state} reduceMotion={Boolean(reduceMotion)} />
                    </span>
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-semibold text-white">{copy.label}</p>
                      <p className="truncate text-[10px] text-white/50">{copy.detail}</p>
                    </div>
                    <button
                      type="button"
                      onClick={() => setVisibility(false)}
                      aria-label="Hide Nimbus companion"
                      title="Hide Nimbus companion"
                      className="flex h-7 w-7 items-center justify-center rounded-full text-white/35 hover:bg-white/[0.06] hover:text-white/75"
                    >
